...
class GPTConfig:
    ...
    vocab_size: int = 50257  # Number of tokens (GPT-2 default)
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        # Flash Attention compatibility (optional, requires specific installation)
...
class MLP(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.gelu = nn.GELU()  # Use GELU activation
...
class GPT(nn.Module):
    ...
    def get_num_params(self, non_embedding=True):
        ...
        if non_embedding:
            n_params -= self.transformer.wpe.weight.numel()  # Subtract positional embeddings
        return n_params
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            ...
            # Reshape logits and targets for cross_entropy loss
            # Logits: (B, T, V) -> (B*T, V)
            # Targets: (B, T) -> (B*T)
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1),
                                   ignore_index=-1)  # ignore_index=-1 useful for padding
        else:
            # Inference-time mini-optimization: only forward the lm_head on the very last position
            # Note: slicing results in logits being shape (B, 1, V)
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        """
        Separate parameters for weight decay. Weight decay is applied to weights,
        but typically not to biases or LayerNorm/embedding weights.
        """
        # start with all of the candidate parameters
        param_dict = {pn: p for pn, p in self.named_parameters()}
        # filter out those that require grad
